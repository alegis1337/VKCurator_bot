...
| `/broadcast <текст>` | Разослать одно сообщение во ВСЕ активные беседы (во всех сообществах) с префиксом «📢 Объявление от куратора:». Отвечает отчётом «доставлено / ошибок». Используется для срочных оповещений (отмена занятий и т.п.) |
